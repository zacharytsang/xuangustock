...
def get_total_raw_data(line,i):
    ...
    try:
        ...
        while x<=count_days:
            ...
            ss1 = vol_1.split(",")
            vol_1_end_price = float(ss1[5])         #交易量，总手
            total_res.append(vol_1_end_price)
            x=x+1
    except Exception,e:
        if exp_sig==1:
        ...
        return
    print "=============",i,">",now_c,"================="
    temp_list = sorted(total_res)
    result_positon = 0
    total_result_len = len(temp_list)
    while result_positon<total_result_len-1:
        result_positon = result_positon + 1
        if temp_list[result_positon-1]>=end_vol:
            break
    print "code is:",result_positon,line,temp_list
    
    if total_result_len-result_positon>0.9*total_result_len:
        lock.acquire()
