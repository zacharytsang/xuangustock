...
def update_data(line,i):
        ...
        try:
            ...
            while start_i<=15:
                ...
                while start_i_tmp<=end_i:
                    print "short_period,long_period",start_i,start_i_tmp
    ...
    lock.acquire()
    now_c=now_c -1
    if max_result[0]==0 and max_result[1]==0 and max_result[2]==0.0:
        print "to short time period!!!!"
    else:
        open("total_eql_check_result_local.txt","a").write(line+" :"+str(max_result[0])+","+str(max_result[1])+","+str(max_result[2])+"\n")
    lock.release() 
    thread.exit_thread()
    print "finished!",i 
...
def get_check_result(data,num_short,num_long,total_len):   #参数说明，所有样本数据，短均线周期，长均线周期，样本数据总长度
    ...
    while y<total_len:
        ...
        if vol_1!=0.0 and vol_2!=0.0 and vol_1<=vol_2 and in_or_out == 1:   #还没有卖出，短均线下穿长均线，没有出错
            lowest_price = get_lowest_price(data,y)
            if(lowest_price==0.0):
                y = y +1
                continue
