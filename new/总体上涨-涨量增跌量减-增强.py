...
def get_percent_eq(date,num,ch):
    ...
    try:
	    ...
	    temp_vol = ll[5] 
            # print start
            eq = eq + float(start)
            price_up_down = price_up_down + float(temp_vol)
            n=n+1
            if n>=num:
                break  
        ...
        return 0.0,10
    print "eq/num,price_up_down/num ",eq/num,price_up_down/num 
    return eq/num,price_up_down/num 
...
def get_total_raw_data(line,i):
    ...
    try:
        ...
        low_len = int(lenth*0.2)
        low_vol_list = temp_list[0:low_len]
        low_eql_vol = 0.0
        for temp_vol in low_vol_list:
            low_eql_vol = low_eql_vol + temp_vol
        low_eql_vol = low_eql_vol/low_len          #取样本交易量最小的20%取平均值，最新的交易量要比这个小
        if low_eql_vol<latest_vol:
            lock.acquire()
            now_c=now_c -1
            lock.release() 
            return ""
    ...
    if tmp_cop>0.5:
        if vol_up > 1.7*vol_down:
            lock.acquire()
            open(fname,"a").write(str(1)+line+"\n") 
            now_c=now_c -1
            lock.release() 
            return
...
fname ="pri_vol_up_down_better"+fnamet+".txt"
