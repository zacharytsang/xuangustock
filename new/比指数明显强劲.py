...
def get_total_raw_data(line,i):
    ...
    global index_data
    global index_calc_result
    global count_days
    global index_total_goup
    # print "++++++",now_c,"++++++3"
    total_data=[]
    lock.acquire()
    # now_c=now_c +1
    ...
    try:
        ...
        if temp_total_len<statics_days:
            ...
            return
        checks = total_data[-1]
        end_price = float(ss[4])
        eql40 = get_percent_eq(total_data,40,1)    #40日均线以下，直接返回，不满足条件
        #if eql40>=end_price:
        #    print "-----------------below 40 eql------------------"
        #    lock.acquire()
        #    now_c=now_c -1
        ...
        while x<=count_days:
            vol_1 = total_data[temp_total_len-1-x]   #当前交易日
            vol_2 = total_data[temp_total_len-1-x-1] #前一个交易日价格
            ss1 = vol_1.split(",")
            ss2 = vol_2.split(",")
            vol_1_end_price = float(ss1[4])
            vol_2_end_price = float(ss2[4])
            temp_result = (vol_1_end_price - vol_2_end_price)/vol_2_end_price
            if temp_result>index_calc_result[x]:
                total_res.append(temp_result)
            x=x+1
    except Exception,e:
        if exp_sig==1:
    ...
    temp_go_up = sum(total_res)
    print "temp go up:",temp_go_up,"index go up:",index_total_goup,"valid:",len(total_res)
    if temp_go_up>index_total_goup and temp_go_up>0 and len(total_res) > 0.75*count_days:
        lock.acquire()
...
#获得指数历史数据http://d.10jqka.com.cn/v2/line/hs_600202/01/2016.js    上证http://d.10jqka.com.cn/v2/line/16_1A0001/01/2016.js
...
count_days = 15    #做比较的天数
index_data = get_full_data("1A0001", statics_days)
if len(index_data)<=statics_days:
    index_data = get_full_data("1A0001", statics_days)
index_total_len = len(index_data)

index_calc_result = []
x = 0
while x<=count_days:
    vol_1 = index_data[index_total_len-1-x]   #当前交易日
    vol_2 = index_data[index_total_len-1-x-1] #前一个交易日价格
    ss1 = vol_1.split(",")
    ss2 = vol_2.split(",")
    vol_1_end_price = float(ss1[4])
    vol_2_end_price = float(ss2[4])
    temp_result = (vol_1_end_price - vol_2_end_price)/vol_2_end_price
    index_calc_result.append(temp_result)
    x = x + 1
index_total_goup = sum(index_calc_result)
print "index total length: ",len(index_calc_result)
print index_calc_result
...
fname ="strong_than_index_"+fnamet
