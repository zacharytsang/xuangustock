...
def get_total_raw_data(line,i):
    ...
    try:
        ...
        end_vol = float(ss[5])
        eql40 = get_percent_eq(total_data,40,1)    #4日均线在4日均线以下，直接返回，不满足条件
        eql4 = get_percent_eq(total_data,4,1)    #4日均线
        if eql40>eql4:
            lock.acquire()
            now_c=now_c -1
            lock.release() 
        ...
        while x<count_days:                                            #check 15 days to 20 days
            ...
            price_len = len(all_swag)-1     #continue down, sum down smaller than -0.05
            exit_sign = 0
            i = 0
            while i<price_len:
                temp_all_down = 0.0
                if all_swag[i]<0:
                    temp_all_down = all_swag[i]
                    start_i = i+1
                    while start_i<price_len:
                        if all_swag[start_i]>=0.0:
                            break
                        temp_all_down = temp_all_down + all_swag[start_i]
                if temp_all_down<-0.05:
                    exit_sign = 1
                i = i + 1
                
                
            print "up_days>0.65*count_days , temp_min",up_days,x,line,temp_min
            if up_days>0.65*x and temp_min>-0.03 and exit_sign == 0:
                ...
                if(c_res>=0.015 and c_res<=0.15) and ((end_price_single-start_price)/start_price)>0.02:
                    lock.acquire()
