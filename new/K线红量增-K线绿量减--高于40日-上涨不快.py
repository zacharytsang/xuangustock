...
def get_total_raw_data(line,i):
    ...
    try:
        ...
        eql10,price_goup = get_percent_eq(total_data,15,1)
        if eql4 <= eql40 or price_goup>0.1 or price_goup<0.0:
            lock.acquire()
            now_c=now_c -1
            lock.release() 
            return ""
    ...
    # if analyze.get_final_percentage_result():
        ...
        # lock.release() 
        # return
    if tmp_cop>=0.7 and vol_up > 1.4*vol_down:
            lock.acquire()
            open(fname,"a").write(str(0)+line+"\n") 
            now_c=now_c -1
            lock.release() 
...
fname ="notfast_40_pri_vol_up_down"+fnamet+".txt"
