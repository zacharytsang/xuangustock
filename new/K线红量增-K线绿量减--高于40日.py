...
def get_total_raw_data(line,i):
    ...
    latest_vol =0.0
    # lock.release() 
    vol_up,vol_down,up_num,down_num = 0.0,0.0,0,0 
    # print "++++++",now_c,"++++++4"
    ...
    try:
        ...
        lenth = len(total_data)-1
        
        start_i = 0
        total_vol = []
        sss=[]
        while start_i<=lenth:
            sss = total_data[start_i].split(",")
            total_vol.append(float(sss[5]))
            start_i = start_i + 1
        temp_list = sorted(total_vol)
        low_len = int(lenth*0.3)
        low_vol_list = temp_list[0:low_len]
        low_eql_vol = 0.0
        for temp_vol in low_vol_list:
            low_eql_vol = low_eql_vol + temp_vol
        low_eql_vol = low_eql_vol/low_len
        
        ii = 3
        while ii<10:
            s=total_data[lenth-ii] 
            ss = s.split(",")
            temp_vol = float(ss[5])
            if  temp_vol<low_eql_vol:
                break
            ii = ii + 1
        if ii>=10 or low_eql_vol<latest_vol:
            lock.acquire()
            now_c=now_c -1
            lock.release() 
            return ""
        ...
        while y<=10:
            tmp_str = total_data[lenth-y]
            tmp_open = 0.0
            tmp_close = 0.0
            tmp_vol = 0.0
            result_list = tmp_str.split(",")
            tmp_open = float(result_list[1])
            tmp_close = float(result_list[4])
            tmp_vol = float(result_list[5])
            if tmp_close>tmp_open:  
                vol_up = vol_up + tmp_vol
                up_num = up_num + 1
            else:
                vol_down = vol_down + tmp_vol
                down_num = down_num + 1
            y = y + 1
        vol_up = vol_up/up_num
        vol_down = vol_down/down_num
        up_num = float(up_num)
        down_num = float(down_num)
    except Exception,e:
        if exp_sig==1:
    ...
    tmp_cop = up_num/(down_num+up_num)
    print "=============",i,">",now_c,"================="
    print "vol_up/up_num,vol_down/down_num,up_num,down_num,tmp_cop"
    print vol_up,vol_down,up_num,down_num,tmp_cop
    # analyze = Analyze(total_eq_volume,total_eq_price,2)
	
    # if analyze.get_final_percentage_result():
        # lock.acquire()
        # open(fname,"a").write(str(0)+line+"\n") 
        # now_c=now_c -1
        # lock.release() 
        # return
    if tmp_cop>=0.6 and vol_up > 1.3*vol_down:
        eql4 = get_percent_eq(total_data,4,1)
        eql40 = get_percent_eq(total_data,40,1)
        if eql4 > eql40:
            lock.acquire()
            open(fname,"a").write(str(0)+line+"\n") 
            now_c=now_c -1
...
fname ="40_pri_vol_up_down"+fnamet+".txt"
#check_codes ="10jqka_eql" + fnamet + ".txt"
# print fname2
