...
def get_percent_eq(date,num,ch):
    ...
    try:
        ...
        return 0.0,10,0
    print "price_up/count,price_down/(num-count)",price_up/count,price_down/(num-count)
    return price_up/count,price_down/(num-count),count    #上涨那些天的平均交易量，下跌那些天的平均交易量，上涨天数
...
def get_total_raw_data(line,i):
    ...
    try:
        ...
        total_eq_volume = []
        #8天的计算结果
	upvol_8,downvol_8,count8 = get_percent_eq(total_data,10,0)
	upvol_10,downvol_10,count10 = get_percent_eq(total_data,10,2)
    ...
    if upvol_8> downvol_8*1.3 and upvol_8> downvol_8*1.3 and count8>4 and count10>4:
        lock.acquire()
