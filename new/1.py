...
def get_total_raw_data(line,i):
    ...
    try:
        ...
        eql15 = get_percent_eq(total_data,LONG_EQL_PERIOD,1)    
        eql10 = get_percent_eq(total_data,SHORT_EQL_PERIOD,1)  
        eql15_3 = get_percent_eq(total_data,LONG_EQL_PERIOD,4)    
        eql10_3 = get_percent_eq(total_data,SHORT_EQL_PERIOD,4) 
        eql15_6 = get_percent_eq(total_data,LONG_EQL_PERIOD,8)    
        eql10_6 = get_percent_eq(total_data,SHORT_EQL_PERIOD,8)  
        if eql40>eql4 or eql15>eql10 or eql15_3>eql10_3 or eql15_6>eql10_6:
            lock.acquire()
            now_c=now_c -1
            lock.release() 
            return
        ...
        while x<=NUM:
            ...
            l = float(ss1[3])
            h = float(ss1[2])
            tmp_open = float(ss1[1])
            tmp_close = float(ss1[4])
            tmp_vol = float(ss1[5])
            ...
            total_price.append(l)
            total_price.append(h)
            #price_switch.append((h-l)/l)
        ...
        while x<count_days:                                               #check 15 days to 20 days
            ...
            if tmp_close>tmp_open:  
                ...
                vol_down = vol_down + tmp_vol
                down_num = down_num + 1
            all_stop_price.append(tmp_close)
            
            if(float(ss1[4])>float(ss1[1])):
                up_days = up_days + 1
            
            price_len = len(all_stop_price)-1   #down per day slow than -0.025
            i = 0
            all_swag = []
            while i<price_len:
            ...
            #temp_swtch = get_switch(price_switch,x)    
            print "up_days>0.65*count_days , temp_min",up_days,x,line,temp_min
            if up_days>0.7*x and 0.5*(vol_up/up_num)>(vol_down/down_num) and temp_min>-0.035 :
...
fname ="(2016-11-21)higher40-movelittle-highest-lowest"+fnamet
