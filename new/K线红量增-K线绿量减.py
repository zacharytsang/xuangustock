...
def get_total_raw_data(line,i):
    ...
    # if analyze.get_final_percentage_result():
        ...
        # return
    if tmp_cop>0.5:
        if vol_up > 2*vol_down:
            lock.acquire()
            open(fname,"a").write(str(1)+line+"\n") 
            now_c=now_c -1
            lock.release() 
            return
        if vol_up > 1.8*vol_down:
            lock.acquire()
            open(fname,"a").write(str(2)+line+"\n") 
            now_c=now_c -1
            lock.release() 
            return
        if vol_up > 1.5*vol_down:
            lock.acquire()
            open(fname,"a").write(str(2)+line+"\n") 
            now_c=now_c -1
            lock.release() 
            return
        # if vol_up > 1.2*vol_down:
            # lock.acquire()
            # open(fname,"a").write(str(4)+line+"\n") 
            # now_c=now_c -1
            # lock.release() 
            # return
...
fname ="pri_vol_up_down"+fnamet+".txt"
...
i=0
check_point = 0
for line in open(check_codes,'r'):#fname2
    line = line.strip()
    if line =="":
        continue
