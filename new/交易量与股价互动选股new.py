...
def get_percent_eq(line,date,num,ch):
    total_data=date
    # n=num
    eq = 0.0
    # start1 = 0.0
    ...
    try:
        ...
        while 1:  
            ...
            price_down = 0.0             
            while 1:
                ll = total_data[tlong-n-checknum-1].split(",")
                start = float(ll[4])
                end = float(ll[1])
                #print "------+++------",ll[0]
                temp_vol = ll[5] 
                if start>end*1.005:
                    count = count + 1 
                    #统计上涨天数的总交易量
                    price_up = price_up + float(temp_vol)
                    #print "+++",ll[0],ll[5] 
                    eq = eq + float(start)
                else:
                    price_down = price_down + float(temp_vol)
                    count2 = count2 +1
                n=n+1
                if n>4:
                    break  
            #print "-----------",price_up,price_down
            if price_up/count>(price_down/count2)*2:
    ...
    temp_result = 0.0
    num = num +1
    temp_result=float(good_count)/float(num)
    print "good_count...num...temp_result",good_count,num,temp_result
    print_log(line+":  good_count...num...temp_result: "+str(good_count)+"--"+str(num)+"--"+str(temp_result))
    return temp_result  #上涨那些天的平均交易量，下跌那些天的平均交易量，上涨天数
...
def get_total_raw_data(line,i):
    ...
    try:
        ...
        total_eq_volume = []
        #8天的计算结果
	upvol_8= get_percent_eq(line,total_data,8,0)
...
def print_log(str):
    ...
    global fname
    lock.acquire()
    open("log_"+fname,"a").write(str+"\n")
    lock.release()
    

this_year = time.strftime('%Y',time.localtime(time.time()))
statics_days = 30  #统计多少天的交易量
